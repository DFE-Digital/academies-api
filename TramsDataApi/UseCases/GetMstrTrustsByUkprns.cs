...
{
    public class GetMstrTrustsByUkprns : IGetMstrTrustsByUkprns
    {
        private readonly ITrustGateway _trustGateway;
        private readonly IGetEstablishments _getEstablishmentsUseCase;

        public GetMstrTrustsByUkprns(ITrustGateway trustGateway, IGetEstablishments getEstablishmentsUseCase)
        {
            _trustGateway = trustGateway;
            _getEstablishmentsUseCase = getEstablishmentsUseCase;
        }

        public IList<MasterTrustResponse> Execute(GetTrustsByUkprnsRequest request)
        {
            ...
            var trusts = _trustGateway.GetMultipleMasterTrustsByGroupId(groupIds).ToList();
            ...
            return distinctGroups.Select(group =>
                    TrustResponseFactory.CreateFromMaster(
                        group,
                        trusts?.FirstOrDefault(trust => trust.GroupID == group.GroupId),
