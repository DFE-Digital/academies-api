...
{
    public class CustomWebApplicationDbContextFactoryCustomization<TProgram> : ICustomization
        where TProgram : class
    {
        public void Customize(IFixture fixture)
        {
            fixture.Customize<CustomWebApplicationDbContextFactory<TProgram>>(composer => composer.FromFactory(() =>
            {

                var factory = new CustomWebApplicationDbContextFactory<TProgram>()
                {
                    SeedData = new Dictionary<Type, Action<DbContext>>
                    {
                        { typeof(MstrContext), context => MstrSeedData((MstrContext)context) },
                        { typeof(MopContext), context => MopSeedData((MopContext)context) }
                    },
        ...
        private static void MstrSeedData(MstrContext mstrContext)
        {
        ...
        private static void MopSeedData(MopContext mopContext)
        {
