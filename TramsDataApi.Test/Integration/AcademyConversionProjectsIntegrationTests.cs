...
namespace TramsDataApi.Test.Integration
{
	...
	public class AcademyConversionProjectsIntegrationTests : IClassFixture<TramsDataApiFactory>, IDisposable
	{
			...
			Patch_request_should_update_an_academy_conversion_project_when_project_entity_already_exists_in_db()
		{
			...
			var expected = CreateExpectedApiResponse(academyConversionProject, updateRequest);
			...
			Get_request_with_state_should_get_response_with_data_as_list_of_academy_conversion_projects_filtered_by_state()
		{
            ...
            GetAcademyConversionSearchModel searchModel = new GetAcademyConversionSearchModel(titleFilter: null,
                count: 10, page: 1, regionUrnsQueryString: null, deliveryOfficerQueryString: null, statusQueryString: states);
		...
		[Fact]
		public async Task Get_request_should_return_response_with_data_having_ukPrn_and_Laestab_set()
		{
		...
		[Fact]
		public async Task Get_request_without_state_should_return_response_with_data_containing_all_records()
		{
			var academyConversionProjects = _fixture.Build<AcademyConversionProject>()
		...
		[Fact]
		public async Task Get_project_filter_parameters()
		{
			var academyConversionProjects = _fixture.Build<AcademyConversionProject>()
				.Without(x => x.Id)
				.CreateMany()
			...
			Assert.Multiple(
				() => response.StatusCode.Should().Be(HttpStatusCode.OK),
				() => content.Statuses.Should().BeEquivalentTo(academyConversionProjects.Select(p => p.ProjectStatus)),
				() => content.AssignedUsers.Should().BeEquivalentTo(academyConversionProjects.Select(p => p.AssignedUserFullName))
			);
		}
		...
		public async Task Get_project_should_return_distinct_filter_parameters()
		{
			...
			var academyConversionProjects = _fixture.Build<AcademyConversionProject>()
				.With(x => x.ProjectStatus, status)
				.With(x => x.AssignedUserFullName, assignedUser)
				.Without(x => x.Id)
				.CreateMany()
				.ToList();
			...
			Assert.Multiple(
				() => response.StatusCode.Should().Be(HttpStatusCode.OK),
				() => content.Statuses.Count.Should().Be(1),
				() => content.Statuses[0].Should().Be(status),
				() => content.AssignedUsers.Count.Should().Be(1),
				() => content.AssignedUsers[0].Should().Be(assignedUser)
			);
		}
		...
		public async Task Get_project_should_return_ordered_status_filter_parameters()
		{
			...
			_dbContext.AcademyConversionProjects.Add(
				_fixture.Build<AcademyConversionProject>().With(x => x.ProjectStatus, status3).Without(x => x.Id).Create());
			_dbContext.AcademyConversionProjects.Add(
				_fixture.Build<AcademyConversionProject>().With(x => x.ProjectStatus, status1).Without(x => x.Id).Create());
			_dbContext.AcademyConversionProjects.Add(
				_fixture.Build<AcademyConversionProject>().With(x => x.ProjectStatus, status2).Without(x => x.Id).Create());
			...
			Assert.Multiple(
				() => response.StatusCode.Should().Be(HttpStatusCode.OK),
				() => content.Statuses[0].Should().Be(status1),
				() => content.Statuses[1].Should().Be(status2),
				() => content.Statuses[2].Should().Be(status3)
			);
		}
		...
		[Fact]
		public async Task Get_request_should_return_response_with_data_having_Upin_and_NewAcademyUrn_set()
		{
			var urns = new List<int> { 1, 2 };
		...
		private AcademyConversionProjectResponse CreateExpectedApiResponse(AcademyConversionProject academyConversionProject, UpdateAcademyConversionProjectRequest updateRequest)
		{
